...
@section('content')
    ...
    <style>
        ...
        .animate-slide-up { animation: slideUp 0.28s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
        ...
        @media (min-width: 640px) {
            .table-status-btn { padding: 6px 18px; font-size: 0.85rem; }
        }
        .table-status-btn.active { color: #fff; }
        ...
        /* ── Collapse ── */
        .activities-collapse { max-height: 0; overflow: hidden; transition: max-height 0.3s cubic-bezier(0.4,0,0.2,1); }
        .activities-collapse.open { max-height: 500px; }
        ...
        .mode-tab.active { background:#fff; color:#2563eb; box-shadow:0 1px 4px rgba(0,0,0,0.1); }
        ...
        .range-preview.has-items { border-color: #93c5fd; background: #eff6ff; }
        .rp-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: #8b90a7; margin-bottom: 6px; }
        .rp-chips { display: flex; flex-wrap: wrap; gap: 5px; max-height: 60px; overflow-y: auto; }
        .rp-chip  { font-size: 11px; font-weight: 600; padding: 3px 8px; background: #2563eb; color: #fff; border-radius: 5px; }
        .rp-chip.dup { background: #ef4444; text-decoration: line-through; opacity: 0.7; }
        .rp-empty { font-size: 12px; color: #8b90a7; font-style: italic; }
    ...
    <div id="historyModal" class="fixed inset-0 z-50 hidden items-end sm:items-center justify-center">
        ...
        <div id="historyContent"
            ...
            {{-- Header --}}
            <div class="px-4 md:px-5 py-3 md:py-4 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
    ...
    <div id="drawer">
        ...
        <div class="tab-panel active" id="panelSingle">
            <form id="addTableForm" class="px-4 md:px-5 pb-5 space-y-4 md:space-y-5">
                <div>
                    <label class="block text-xs md:text-sm font-semibold text-gray-700 mb-2" for="tableNumber">Table Number</label>
                ...
                <div>
                    <label class="block text-xs md:text-sm font-semibold text-gray-700 mb-2" for="spaceArea">Space Area</label>
        ...
        <div class="tab-panel" id="panelBulk">
            <form id="bulkForm" onsubmit="submitBulk(event)" class="px-4 md:px-5 pb-5 space-y-4 md:space-y-5">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Prefix + number must stay under 5 chars (e.g. "T" + "16" = "T16").</p>
                </div>
                <div class="bulk-row">
                    <div>
                        <label class="block text-xs md:text-sm font-semibold text-gray-700 mb-2">From</label>
                        <input class="field-input text-sm" type="number" id="bulkFrom" min="1" max="999" placeholder="1" oninput="updatePreview()">
                    </div>
                    <div>
                        <label class="block text-xs md:text-sm font-semibold text-gray-700 mb-2">To</label>
                        <input class="field-input text-sm" type="number" id="bulkTo"   min="1" max="999" placeholder="20" oninput="updatePreview()">
                    </div>
                </div>
    ...
    <script>
        ...
        function toggleHistory() {
            ...
            if (historyModal.classList.contains('hidden')) {
                ...
                let totalOrders = table.orders.length, totalCompleted = 0, revenue = 0;
                for (const o of table.orders) {
                    if (o.status === 'completed') { totalCompleted++; revenue += o.total_amount; }
                }
    ...
    <script>
        ...
        function tableCard(table) {
            ...
            }[table.status] || { bg:'bg-white', border:'border-gray-200', badge:'bg-gray-100 text-gray-600', ring:'' };
        ...
        // ── Activity config ──
        const ACT_CFG = {
            created:      { bg:'bg-blue-100',   icon:'text-blue-500',   path:'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
            item_added:   { bg:'bg-green-100',  icon:'text-green-600',  path:'M12 6v6m0 0v6m0-6h6m-6 0H6' },
            item_removed: { bg:'bg-orange-100', icon:'text-orange-500', path:'M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16' },
            qty_updated:  { bg:'bg-blue-100',   icon:'text-blue-500',   path:'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
            cancelled:    { bg:'bg-red-100',    icon:'text-red-500',    path:'M6 18L18 6M6 6l12 12' },
            checkout:     { bg:'bg-purple-100', icon:'text-purple-600', path:'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
        };
        ...
        function buildActivitySentence(act) {
            ...
            switch (act.action) {
                case 'created':      return `${actor} created the order`;
                case 'item_added':   return `${actor} added <span class="font-semibold text-gray-800">${meta.name||'item'}</span>${meta.quantity?` <span class="text-blue-600 font-semibold">×${meta.quantity}</span>`:''}`;
                case 'item_removed': return `${actor} removed <span class="font-semibold text-gray-800">${meta.name||'item'}</span>`;
                case 'qty_updated':  return `${actor} updated <span class="font-semibold text-gray-800">${meta.name||'item'}</span> qty${meta.from!==undefined?` <span class="text-gray-400">(${meta.from} → <span class="text-blue-600 font-semibold">${meta.to}</span>)</span>`:''}`;
                case 'cancelled':    return `${actor} cancelled order${meta.reason?` <span class="text-gray-400">— ${meta.reason}</span>`:''}`;
                case 'checkout':     return `${actor} completed checkout`;
                default:             return `${actor} ${act.action.replace(/_/g,' ')}`;
            }
        }
    ...
    <script>
        ...
        form.addEventListener('submit', async function(e) {
            e.preventDefault();
            const payload = { table_number: form.table_number.value, area_name: form.area_name.value };
            try {
