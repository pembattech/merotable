<style>
    ...
    .blur-btn:hover { background: rgba(108, 108, 108, 0.458); transform: scale(1.05); }
    ...
    @media (min-width: 400px) { .status-pill { gap: 8px; padding: 8px 14px; } }
    .status-pill.absent       { border-color: #fecaca; background: #fef2f2; }
    .status-pill.checked-out  { border-color: #bfdbfe; background: #eff6ff; }

    .status-indicator { position: relative; width: 8px; height: 8px; flex-shrink: 0; }
    .status-dot {
        width: 8px; height: 8px; border-radius: 50%;
        background: #16a34a; position: absolute; inset: 0;
    }
    .status-dot.absent { background: #dc2626; }
    .status-dot.out    { background: #2563eb; }
    .status-ring {
        width: 8px; height: 8px; border-radius: 50%;
    ...
    .status-ring.absent { border-color: #dc2626; }
    .status-ring.out    { border-color: #2563eb; animation: none; }
    ...
    .status-label-sub {
        font-size: 9px;
        font-weight: 500; color: #4ade80; line-height: 1;
        text-transform: uppercase; letter-spacing: 0.05em;
    }
    @media (min-width: 400px) { .status-label-sub { font-size: 10px; } }
    .status-pill.absent      .status-label-sub { color: #f87171; }
    .status-pill.checked-out .status-label-sub { color: #60a5fa; }

    .status-label-main {
        font-size: 11px;
        font-weight: 700; color: #15803d; line-height: 1;
        text-transform: uppercase; letter-spacing: 0.05em;
    }
    @media (min-width: 400px) { .status-label-main { font-size: 12px; } }
    .status-pill.absent      .status-label-main { color: #b91c1c; }
    .status-pill.checked-out .status-label-main { color: #1d4ed8; }
    ...
    @media (min-width: 400px) { .checkin-btn { padding: 16px; font-size: 16px; } }
    .checkin-btn.in   { background: #16a34a; color: #fff; box-shadow: 0 4px 16px rgba(22,163,74,0.25); }
    .checkin-btn.out  { background: #dc2626; color: #fff; box-shadow: 0 4px 16px rgba(220,38,38,0.22); }
    .checkin-btn.done { background: #e5e7eb; color: #9ca3af; cursor: default; }
    .checkin-btn:not(.done):hover  { transform: translateY(-1px); filter: brightness(1.07); }
    .checkin-btn:not(.done):active { transform: translateY(0); }
    ...
    @media (min-width: 400px) { .stat-card { padding: 14px 10px; } }
    .stat-card:hover { border-color: #e0e3ef; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
    ...
    @media (min-width: 360px) { .cal-day { font-size: 11px; border-radius: 7px; } }
    @media (min-width: 400px) { .cal-day { font-size: 12px; border-radius: 8px; } }
    ...
    .cal-day.today   { outline: 2px solid #2563eb; outline-offset: 1px; }
    ...
    @media (min-width: 420px) { #drawer { width: 380px; } }
    ...
    #drawerBackdrop.open { display: block; opacity: 1; }
</style>
    ...
    <div class="p-4 pb-12">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-4">
            <div class="flex justify-between items-center mb-4 sm:mb-5">
                ...
                <div class="text-right">
                    ...
                    <p class="text-xs sm:text-sm font-bold text-gray-800 mt-0.5 tabular-nums" id="liveClock">--:--:--</p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5 mb-4">
            <div class="flex justify-between items-start gap-2 mb-3 sm:mb-4">
                ...
                <span class="text-xs font-bold text-blue-600 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-full whitespace-nowrap flex-shrink-0">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5">
            ...
            <div class="flex flex-wrap gap-2 sm:gap-3 text-xs font-semibold mb-3 sm:mb-4">
                <span class="flex items-center gap-1">
                    <span class="w-2.5 h-2.5 rounded-sm bg-green-100 inline-block border border-green-200"></span>Present
                </span>
                <span class="flex items-center gap-1">
                    <span class="w-2.5 h-2.5 rounded-sm bg-yellow-100 inline-block border border-yellow-200"></span>Late
                </span>
                <span class="flex items-center gap-1">
...
<script>
    ...
    // ── State ─────────────────────────────────────────────────────
    const todayKey   = `hc-att-${new Date().toISOString().slice(0,10)}`;
    let att          = JSON.parse(localStorage.getItem(todayKey)) || { checkIn: null, checkOut: null, totalHours: 0 };
    ...
    async function handleAttendance() {
        ...
        if (!att.checkIn) {
            ...
            method = 'POST';
            att.checkIn = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
            ...
            att.checkOut   = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
    ...
    function renderCalendar() {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            else if (status === 'present') { cls += 'present'; title = 'Present'; }
            else if (status === 'late')    { cls += 'late';    title = 'Late'; }
            else if (status === 'absent')  { cls += 'absent';  title = 'Absent'; }
            else                           { cls += 'future'; }
